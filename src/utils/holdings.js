...
const process = (trades) => {
    ...
    const {openShares, totalCost, totalShares} = trades.reduce((acc, curr) => {
        if (curr.type) {
    ...
    let buyTrades = newTrades.filter(trade => trade.type).reduce((acc, obj) => { // O(N)
        acc[obj.id] = obj;
        return acc;
      }, {})
    ...
    sellTrades.map(sellTrade => { // O(N)
        
        // Finding weight & returns
        const closedWeight = sellTrade.qty / closedShares
        
        const net_r = (sellTrade.price - buyTrades[sellTrade.closeId].price) * sellTrade.qty
        const r = (sellTrade.price - buyTrades[sellTrade.closeId].price )/buyTrades[sellTrade.closeId].price
        ...
        // array 
        buyTrades[sellTrade.closeId].qty -= sellTrade.qty
    ...
    return {
        ...
        mktPrice,
        unrealizedReturns: Number(((mktPrice*100 - avgUnrealizedCostBasis)/avgUnrealizedCostBasis).toFixed(2)),
